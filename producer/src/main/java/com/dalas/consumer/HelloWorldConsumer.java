...
public class HelloWorldConsumer {
    ...
    public static void main(String[] args) {
        ...
        kaProperties.put("group.id", "fuel_iot_helloconsumer");
    ...
    private void consume (Properties kaProperties) {
        try (KafkaConsumer<Long, Transaction> consumer = new KafkaConsumer<>(kaProperties)) {
            consumer.subscribe(List.of(TOPIC_NAME));

            while (keepConsuming) {
                ConsumerRecords<Long, Transaction> records = consumer.poll(Duration.ofMillis(250));
                for (ConsumerRecord<Long, Transaction> record : records) {
                    log.info("fuel_iot_info offset = {}, fuel_iot_key = {}, fuel_iot_value = {}",
